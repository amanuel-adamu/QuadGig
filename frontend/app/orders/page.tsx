...
function getAvailableActions(status: Order["status"]): OrderAction[] {
  switch (status) {
    case "requested":
      return [
        { label: "Seller: Accept", status: "accepted", className: "bg-emerald-600 hover:bg-emerald-700" },
        { label: "Cancel", status: "cancelled", className: "bg-rose-600 hover:bg-rose-700" },
      ];
    case "accepted":
      return [
        { label: "Seller: Start Task", status: "in_progress", className: "bg-indigo-600 hover:bg-indigo-700" },
        { label: "Cancel", status: "cancelled", className: "bg-rose-600 hover:bg-rose-700" },
      ];
    case "in_progress":
      return [
        { label: "Seller: Mark Delivered", status: "delivered", className: "bg-purple-600 hover:bg-purple-700" },
        { label: "Cancel", status: "cancelled", className: "bg-rose-600 hover:bg-rose-700" },
      ];
    case "delivered":
      return [
        { label: "Buyer: Confirm & Release Payout", status: "confirmed", className: "bg-green-600 hover:bg-green-700" },
        { label: "Dispute", status: "disputed", className: "bg-orange-600 hover:bg-orange-700" },
      ];
    case "disputed":
      return [
        { label: "Buyer: Confirm & Release Payout", status: "confirmed", className: "bg-green-600 hover:bg-green-700" },
...
export default function OrdersPage() {
  ...
  const availableActions = activeOrder ? getAvailableActions(activeOrder.status) : [];
